...
class MerchantAdmin(admin.ModelAdmin):
    ...
    def approve_merchants(self, request, queryset):
        ...
        if approved_count > 0:
            self.message_user(request, f'{approved_count} merchants were successfully approved and activated.')
        if already_approved_count > 0:
            self.message_user(request, f'{already_approved_count} merchants were already approved.', level=messages.WARNING)