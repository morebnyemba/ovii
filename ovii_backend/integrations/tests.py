...
class WhatsAppTemplateSyncTestCase(TestCase):
    ...
    def test_convert_template_to_meta_format(self):
        ...
        self.assertIsNotNone(buttons_component)
        self.assertIn("buttons", buttons_component)
        self.assertEqual(len(buttons_component["buttons"]), 1)
        
        # Verify OTP button structure
        otp_button = buttons_component["buttons"][0]
        self.assertEqual(otp_button["type"], "OTP")
        self.assertEqual(otp_button["otp_type"], "COPY_CODE")
        # Text field should not be present (auto-generated by Meta)
        self.assertNotIn("text", otp_button)
