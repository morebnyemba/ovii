...
class EcoCashWithdrawalSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not user.has_set_pin:
            raise serializers.ValidationError({"pin": "You must set a transaction PIN before making a withdrawal."})
...
class PaynowWebhookView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            tx_to_update = Transaction.objects.select_related('wallet__user').get(id=transaction_id, status=Transaction.Status.PENDING)
        except Transaction.DoesNotExist:
            logging.warning(f"Paynow webhook received for unknown or already processed transaction: {transaction_id}")
            return Response(status=status.HTTP_404_NOT_FOUND)

        if payment_status == 'paid':
            with transaction.atomic():
                wallet_locked = Wallet.objects.select_for_update().get(pk=tx_to_update.wallet.pk)
                ...
                tx_to_update.description = f"Paynow top-up successful. Ref: {paynow_reference}"
                tx_to_update.save()
                send_realtime_notification.delay(tx_to_update.wallet.user.id, f"Your wallet has been topped up with ${tx_to_update.amount}.")
        else:
            ...
            tx_to_update.save()
            send_realtime_notification.delay(tx_to_update.wallet.user.id, f"Your wallet top-up of ${tx_to_update.amount} failed.")
