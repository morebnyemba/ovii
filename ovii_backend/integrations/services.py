...
class EcoCashClient:
    ...
    def request_c2b_payment(self, phone_number: str, amount: Decimal, reference: str) -> dict:
        """
        ...
        try:
            response = requests.post(url, json=payload, headers=self._get_auth_headers(), timeout=15)
    ...
    def send_b2c_payment(self, phone_number: str, amount: Decimal, reference: str) -> dict:
        """
        ...
        try:
            response = requests.post(url, json=payload, headers=self._get_auth_headers(), timeout=20)
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
...
class PaynowClient:
    ...
    def create_transaction(self, reference: str, amount: Decimal, user_email: str) -> dict:
        """
        ...
        try:
            response = requests.post(self.initiate_transaction_url, data=payload, timeout=20)
            response.raise_for_status()
            ...
            if parsed_response.get("status", "").lower() != "ok":
                error_message = parsed_response.get("error", "Unknown error from Paynow.")
                raise Exception(error_message)

            return parsed_response

        except requests.RequestException as e:
            logger.error(f"Paynow initiate transaction request failed for ref {reference}: {e}")
            raise
