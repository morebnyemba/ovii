...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    receiver = serializers.CharField(
        source="receiver_identifier", read_only=True, allow_null=True
    )
...
class TransactionCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            destination_user = OviiUser.objects.get(
            ...
            raise serializers.ValidationError(
                {
                    "destination_phone_number": "User with this phone number does not have a wallet."
                }
            )
        ...
        if user == destination_user:
            raise serializers.ValidationError(
                {"destination_phone_number": "You cannot send money to yourself."}
            )

        # Validate the transaction PIN
        if not user.has_set_pin:
            raise serializers.ValidationError(
                {"pin": "You must set a transaction PIN before making transfers."}
            )
...
class CustomerCashOutRequestSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            agent = Agent.objects.select_related("user__wallet").get(
                agent_code__iexact=agent_code, is_approved=True
            )
            ...
            raise serializers.ValidationError(
                {"agent_code": "An approved agent with this code could not be found."}
            )
        ...
        if not customer_user.has_set_pin:
            raise serializers.ValidationError(
                {
                    "pin": "You must set your transaction PIN before performing this action."
                }
            )
...
class AgentCashInSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            customer_user = OviiUser.objects.get(
                phone_number=customer_phone_number, role=OviiUser.Role.CUSTOMER
            )
            ...
            raise serializers.ValidationError(
                {
                    "customer_phone_number": "A customer with this phone number does not have a wallet."
                }
            )
        ...
        if agent_user == customer_user:
            raise serializers.ValidationError(
                {"customer_phone_number": "You cannot perform a cash-in for yourself."}
            )
...
class MerchantPaymentRequestSerializer(serializers.Serializer):
    ...
    def validate_customer_phone_number(self, value):
        ...
        try:
            customer_user = OviiUser.objects.get(
                phone_number=value, role=OviiUser.Role.CUSTOMER
            )
            if not hasattr(customer_user, "wallet"):
                raise serializers.ValidationError(
                    "This customer does not have an active wallet."
                )
        except OviiUser.DoesNotExist:
            raise serializers.ValidationError(
                "A customer with this phone number was not found."
            )
        return value
...
class ApproveMerchantPaymentSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not customer_user.has_set_pin:
            raise serializers.ValidationError(
                {"pin": "You must set your transaction PIN before approving payments."}
            )
        if not customer_user.check_pin(pin):
            raise serializers.ValidationError({"pin": "Incorrect transaction PIN."})
