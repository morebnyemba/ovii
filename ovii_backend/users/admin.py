                ...
                if user.phone_number:
                    try:
                        ...
                        send_whatsapp_template(
                            ...
                            variables={
                                "verification_level": "Identity Verified"
                            }
                        )
                    except Exception as e:
                        logger.error(f"Failed to send WhatsApp KYC approval to {user.phone_number}: {e}")
                ...
                if user.phone_number:
                    try:
                        ...
                        send_whatsapp_template(
                            ...
                            template_name="kyc_approved",
                            variables={
                                "verification_level": "Address Verified"
                            }
                        )
                    except Exception as e:
                        logger.error(f"Failed to send WhatsApp KYC approval to {user.phone_number}: {e}")
        ...
        for document in queryset.filter(status=DocumentStatus.PENDING):
            ...
            if user.phone_number:
                try:
                    ...
                    logger.error(f"Failed to send WhatsApp KYC rejection to {user.phone_number}: {e}")
